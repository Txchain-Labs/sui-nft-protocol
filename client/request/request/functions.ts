...
export function migrateAsPub(
  ...
  return txb.moveCall({
    ...
    typeArguments: [typeArg],
    arguments: [obj(txb, args.policy), obj(txb, args.pub)],
  });
}
...
export interface Confirm_Args {
  completed: Array<ObjectArg> | TransactionArgument;
  rules: ObjectArg;
}

export function confirm_(txb: TransactionBlock, args: Confirm_Args) {
  return txb.moveCall({
    target: `${PUBLISHED_AT}::request::confirm_`,
    arguments: [
      vector(txb, `0x1::type_name::TypeName`, args.completed),
      obj(txb, args.rules),
    ],
  });
}
...
export function enforceRule(
  ...
  return txb.moveCall({
    ...
    arguments: [
      obj(txb, args.self),
      obj(txb, args.cap),
      generic(txb, `${typeArgs[2]}`, args.state),
    ],
  });
}
...
export function newPolicyWithType(
  ...
  return txb.moveCall({
    ...
    typeArguments: typeArgs,
    arguments: [
      generic(txb, `${typeArgs[1]}`, args.witness),
      obj(txb, args.publisher),
    ],
  });
}

export function policyCapFor(txb: TransactionBlock, policy: ObjectArg) {
  return txb.moveCall({
