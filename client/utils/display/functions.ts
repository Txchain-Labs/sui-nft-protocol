...
export function fromVec(
  ...
  return txb.moveCall({
    ...
    arguments: [pure(txb, vec, `vector<0x1::string::String>`)],
  });
}

export function fromVecMap(txb: TransactionBlock, vec: ObjectArg) {
  return txb.moveCall({
...
export interface FromVecMapRefArgs {
  vec: ObjectArg;
  isString: boolean | TransactionArgument;
}

export function fromVecMapRef(txb: TransactionBlock, args: FromVecMapRefArgs) {
  return txb.moveCall({
    target: `${PUBLISHED_AT}::display::from_vec_map_ref`,
    arguments: [obj(txb, args.vec), pure(txb, args.isString, `bool`)],
  });
}
...
export function fromVecUtf8(
  ...
  return txb.moveCall({
    ...
    arguments: [pure(txb, vec, `vector<vector<u8>>`)],
  });
}
