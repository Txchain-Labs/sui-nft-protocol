...
export function isSizedVec(type: string): boolean {
  ...
  return type.startsWith(
    "0x859eb18bd5b5e8cc32deb6dfb1c39941008ab3c6e27f0b8ce2364be7102bb7cb::sized_vec::SizedVec<",
  );
}
...
export class SizedVec<Element extends TypeArgument> implements StructClass {
  ...
  static reified<Element extends Reified<TypeArgument, any>>(
    ...
    return {
      ...
      fromFields: (fields: Record<string, any>) =>
        SizedVec.fromFields(Element, fields),
      fromFieldsWithTypes: (item: FieldsWithTypes) =>
        SizedVec.fromFieldsWithTypes(Element, item),
      fromBcs: (data: Uint8Array) => SizedVec.fromBcs(Element, data),
      bcs: SizedVec.bcs(toBcs(Element)),
      fromJSONField: (field: any) => SizedVec.fromJSONField(Element, field),
      fromJSON: (json: Record<string, any>) => SizedVec.fromJSON(Element, json),
      fromSuiParsedData: (content: SuiParsedData) =>
        SizedVec.fromSuiParsedData(Element, content),
      fetch: async (client: SuiClient, id: string) =>
        SizedVec.fetch(client, Element, id),
      new: (fields: SizedVecFields<ToTypeArgument<Element>>) => {
        return new SizedVec([extractType(Element)], fields);
      },
  ...
  static phantom<Element extends Reified<TypeArgument, any>>(
    ...
    return phantom(SizedVec.reified(Element));
  }
  ...
  static get bcs() {
    return <Element extends BcsType<any>>(Element: Element) =>
      bcs.struct(`SizedVec<${Element.name}>`, {
        capacity: bcs.u64(),
  ...
  static fromFields<Element extends Reified<TypeArgument, any>>(
    ...
    return SizedVec.reified(typeArg).new({
  ...
  static fromJSON<Element extends Reified<TypeArgument, any>>(
    ...
    assertReifiedTypeArgsMatch(
      composeSuiType(SizedVec.$typeName, extractType(typeArg)),
      json.$typeArgs,
      [typeArg],
    );
  ...
  static fromSuiParsedData<Element extends Reified<TypeArgument, any>>(
    ...
    if (!isSizedVec(content.type)) {
      throw new Error(
        `object at ${(content.fields as any).id} is not a SizedVec object`,
      );
    }
    return SizedVec.fromFieldsWithTypes(typeArg, content);
  }
  ...
  static async fetch<Element extends Reified<TypeArgument, any>>(
    ...
    if (res.error) {
      throw new Error(
        `error fetching SizedVec object at id ${id}: ${res.error.code}`,
      );
    }
    if (
      res.data?.bcs?.dataType !== "moveObject" ||
      !isSizedVec(res.data.bcs.type)
    ) {
