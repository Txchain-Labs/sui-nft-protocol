...
export function isAllowlistRule(type: string): boolean {
  ...
  return (
    type ===
    "0xbc3df36be17f27ac98e3c839b2589db8475fa07b20657b08e8891e3aaf5ee5f9::transfer_allowlist::AllowlistRule"
  );
}
...
export class AllowlistRule implements StructClass {
  ...
  static reified(): AllowlistRuleReified {
    return {
      ...
      fromFields: (fields: Record<string, any>) =>
        AllowlistRule.fromFields(fields),
      fromFieldsWithTypes: (item: FieldsWithTypes) =>
        AllowlistRule.fromFieldsWithTypes(item),
      fromBcs: (data: Uint8Array) => AllowlistRule.fromBcs(data),
      bcs: AllowlistRule.bcs,
      fromJSONField: (field: any) => AllowlistRule.fromJSONField(field),
      fromJSON: (json: Record<string, any>) => AllowlistRule.fromJSON(json),
      fromSuiParsedData: (content: SuiParsedData) =>
        AllowlistRule.fromSuiParsedData(content),
      fetch: async (client: SuiClient, id: string) =>
        AllowlistRule.fetch(client, id),
      new: (fields: AllowlistRuleFields) => {
        return new AllowlistRule([], fields);
      },
  ...
  static fromFields(fields: Record<string, any>): AllowlistRule {
    return AllowlistRule.reified().new({
      dummyField: decodeFromFields("bool", fields.dummy_field),
    });
  }

  static fromFieldsWithTypes(item: FieldsWithTypes): AllowlistRule {
  ...
  static fromBcs(data: Uint8Array): AllowlistRule {
    return AllowlistRule.fromFields(AllowlistRule.bcs.parse(data));
  }
  ...
  static fromJSONField(field: any): AllowlistRule {
    return AllowlistRule.reified().new({
      dummyField: decodeFromJSONField("bool", field.dummyField),
    });
  }

  static fromJSON(json: Record<string, any>): AllowlistRule {
  ...
  static fromSuiParsedData(content: SuiParsedData): AllowlistRule {
    if (content.dataType !== "moveObject") {
      throw new Error("not an object");
    }
    if (!isAllowlistRule(content.type)) {
      throw new Error(
        `object at ${(content.fields as any).id} is not a AllowlistRule object`,
      );
    }
    return AllowlistRule.fromFieldsWithTypes(content);
  }

  static async fetch(client: SuiClient, id: string): Promise<AllowlistRule> {
    ...
    if (res.error) {
      throw new Error(
        `error fetching AllowlistRule object at id ${id}: ${res.error.code}`,
      );
    }
    if (
      res.data?.bcs?.dataType !== "moveObject" ||
      !isAllowlistRule(res.data.bcs.type)
    ) {
