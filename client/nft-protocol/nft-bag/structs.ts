...
export function isKey(type: string): boolean {
  ...
  return (
    type ===
    "0xbc3df36be17f27ac98e3c839b2589db8475fa07b20657b08e8891e3aaf5ee5f9::nft_bag::Key"
  );
}
...
export class Key implements StructClass {
  ...
  static fromFields(fields: Record<string, any>): Key {
    return Key.reified().new({ id: decodeFromFields(ID.reified(), fields.id) });
  }
  ...
  static async fetch(client: SuiClient, id: string): Promise<Key> {
    ...
    if (res.error) {
      throw new Error(
        `error fetching Key object at id ${id}: ${res.error.code}`,
      );
    }
...
export function isNftBag(type: string): boolean {
  ...
  return (
    type ===
    "0xbc3df36be17f27ac98e3c839b2589db8475fa07b20657b08e8891e3aaf5ee5f9::nft_bag::NftBag"
  );
}

export interface NftBagFields {
  id: ToField<UID>;
  authorities: ToField<Vector<TypeName>>;
  nfts: ToField<VecMap<ID, "u64">>;
}
...
export class NftBag implements StructClass {
  ...
  readonly authorities: ToField<Vector<TypeName>>;
  readonly nfts: ToField<VecMap<ID, "u64">>;
  ...
  static reified(): NftBagReified {
    return {
      ...
      typeArgs: [] as [],
      reifiedTypeArgs: [],
      fromFields: (fields: Record<string, any>) => NftBag.fromFields(fields),
      fromFieldsWithTypes: (item: FieldsWithTypes) =>
        NftBag.fromFieldsWithTypes(item),
      fromBcs: (data: Uint8Array) => NftBag.fromBcs(data),
      bcs: NftBag.bcs,
      fromJSONField: (field: any) => NftBag.fromJSONField(field),
      fromJSON: (json: Record<string, any>) => NftBag.fromJSON(json),
      fromSuiParsedData: (content: SuiParsedData) =>
        NftBag.fromSuiParsedData(content),
      fetch: async (client: SuiClient, id: string) => NftBag.fetch(client, id),
      new: (fields: NftBagFields) => {
        return new NftBag([], fields);
      },
      kind: "StructClassReified",
    };
  }
  ...
  static get bcs() {
    return bcs.struct("NftBag", {
      id: UID.bcs,
      authorities: bcs.vector(TypeName.bcs),
      nfts: VecMap.bcs(ID.bcs, bcs.u64()),
    });
  }

  static fromFields(fields: Record<string, any>): NftBag {
    return NftBag.reified().new({
      ...
      nfts: decodeFromFields(VecMap.reified(ID.reified(), "u64"), fields.nfts),
    });
  }

  static fromFieldsWithTypes(item: FieldsWithTypes): NftBag {
    ...
    return NftBag.reified().new({
      ...
      nfts: decodeFromFieldsWithTypes(
        VecMap.reified(ID.reified(), "u64"),
        item.fields.nfts,
      ),
    });
  }

  static fromBcs(data: Uint8Array): NftBag {
    return NftBag.fromFields(NftBag.bcs.parse(data));
  }

  toJSONField() {
    return {
      id: this.id,
      authorities: fieldToJSON<Vector<TypeName>>(
        `vector<0x1::type_name::TypeName>`,
        this.authorities,
      ),
  ...
  static fromJSONField(field: any): NftBag {
    return NftBag.reified().new({
  ...
  static fromJSON(json: Record<string, any>): NftBag {
  ...
  static fromSuiParsedData(content: SuiParsedData): NftBag {
    if (content.dataType !== "moveObject") {
      throw new Error("not an object");
    }
    if (!isNftBag(content.type)) {
      throw new Error(
        `object at ${(content.fields as any).id} is not a NftBag object`,
      );
    }
    return NftBag.fromFieldsWithTypes(content);
  }

  static async fetch(client: SuiClient, id: string): Promise<NftBag> {
    const res = await client.getObject({
      id,
      options: {
        showBcs: true,
    ...
    if (res.error) {
      throw new Error(
        `error fetching NftBag object at id ${id}: ${res.error.code}`,
      );
    }
    if (
      res.data?.bcs?.dataType !== "moveObject" ||
      !isNftBag(res.data.bcs.type)
    ) {
