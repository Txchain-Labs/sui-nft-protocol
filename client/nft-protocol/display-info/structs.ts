...
export function isDisplayInfo(type: string): boolean {
  ...
  return (
    type ===
    "0xbc3df36be17f27ac98e3c839b2589db8475fa07b20657b08e8891e3aaf5ee5f9::display_info::DisplayInfo"
  );
}
...
export class DisplayInfo implements StructClass {
  ...
  static reified(): DisplayInfoReified {
    return {
      ...
      fromFields: (fields: Record<string, any>) =>
        DisplayInfo.fromFields(fields),
      fromFieldsWithTypes: (item: FieldsWithTypes) =>
        DisplayInfo.fromFieldsWithTypes(item),
      fromBcs: (data: Uint8Array) => DisplayInfo.fromBcs(data),
      bcs: DisplayInfo.bcs,
      fromJSONField: (field: any) => DisplayInfo.fromJSONField(field),
      fromJSON: (json: Record<string, any>) => DisplayInfo.fromJSON(json),
      fromSuiParsedData: (content: SuiParsedData) =>
        DisplayInfo.fromSuiParsedData(content),
      fetch: async (client: SuiClient, id: string) =>
        DisplayInfo.fetch(client, id),
      new: (fields: DisplayInfoFields) => {
        return new DisplayInfo([], fields);
      },
  ...
  static fromFields(fields: Record<string, any>): DisplayInfo {
  ...
  static fromFieldsWithTypes(item: FieldsWithTypes): DisplayInfo {
  ...
  static fromBcs(data: Uint8Array): DisplayInfo {
    return DisplayInfo.fromFields(DisplayInfo.bcs.parse(data));
  }
  ...
  static fromJSONField(field: any): DisplayInfo {
    return DisplayInfo.reified().new({
  ...
  static fromJSON(json: Record<string, any>): DisplayInfo {
  ...
  static fromSuiParsedData(content: SuiParsedData): DisplayInfo {
    if (content.dataType !== "moveObject") {
      throw new Error("not an object");
    }
    if (!isDisplayInfo(content.type)) {
      throw new Error(
        `object at ${(content.fields as any).id} is not a DisplayInfo object`,
      );
    }
    return DisplayInfo.fromFieldsWithTypes(content);
  }

  static async fetch(client: SuiClient, id: string): Promise<DisplayInfo> {
    ...
    if (res.error) {
      throw new Error(
        `error fetching DisplayInfo object at id ${id}: ${res.error.code}`,
      );
    }
    if (
      res.data?.bcs?.dataType !== "moveObject" ||
      !isDisplayInfo(res.data.bcs.type)
    ) {
