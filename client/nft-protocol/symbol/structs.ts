...
export function isSymbol(type: string): boolean {
  ...
  return (
    type ===
    "0xbc3df36be17f27ac98e3c839b2589db8475fa07b20657b08e8891e3aaf5ee5f9::symbol::Symbol"
  );
}
...
export class Symbol implements StructClass {
  ...
  static reified(): SymbolReified {
    return {
      ...
      fromFieldsWithTypes: (item: FieldsWithTypes) =>
        Symbol.fromFieldsWithTypes(item),
      ...
      fromSuiParsedData: (content: SuiParsedData) =>
        Symbol.fromSuiParsedData(content),
  ...
  static fromSuiParsedData(content: SuiParsedData): Symbol {
    ...
    if (!isSymbol(content.type)) {
      throw new Error(
        `object at ${(content.fields as any).id} is not a Symbol object`,
      );
    }
  ...
  static async fetch(client: SuiClient, id: string): Promise<Symbol> {
    ...
    if (res.error) {
      throw new Error(
        `error fetching Symbol object at id ${id}: ${res.error.code}`,
      );
    }
    if (
      res.data?.bcs?.dataType !== "moveObject" ||
      !isSymbol(res.data.bcs.type)
    ) {
