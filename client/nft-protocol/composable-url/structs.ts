...
export function isWitness(type: string): boolean {
  ...
  return (
    type ===
    "0xbc3df36be17f27ac98e3c839b2589db8475fa07b20657b08e8891e3aaf5ee5f9::composable_url::Witness"
  );
}
...
export function isComposableUrl(type: string): boolean {
  ...
  return (
    type ===
    "0xbc3df36be17f27ac98e3c839b2589db8475fa07b20657b08e8891e3aaf5ee5f9::composable_url::ComposableUrl"
  );
}
...
export class ComposableUrl implements StructClass {
  ...
  static reified(): ComposableUrlReified {
    return {
      ...
      fromFields: (fields: Record<string, any>) =>
        ComposableUrl.fromFields(fields),
      fromFieldsWithTypes: (item: FieldsWithTypes) =>
        ComposableUrl.fromFieldsWithTypes(item),
      fromBcs: (data: Uint8Array) => ComposableUrl.fromBcs(data),
      bcs: ComposableUrl.bcs,
      fromJSONField: (field: any) => ComposableUrl.fromJSONField(field),
      fromJSON: (json: Record<string, any>) => ComposableUrl.fromJSON(json),
      fromSuiParsedData: (content: SuiParsedData) =>
        ComposableUrl.fromSuiParsedData(content),
      fetch: async (client: SuiClient, id: string) =>
        ComposableUrl.fetch(client, id),
      new: (fields: ComposableUrlFields) => {
        return new ComposableUrl([], fields);
      },
      kind: "StructClassReified",
    };
  }
  ...
  static fromFields(fields: Record<string, any>): ComposableUrl {
    return ComposableUrl.reified().new({
      url: decodeFromFields(Url.reified(), fields.url),
    });
  }

  static fromFieldsWithTypes(item: FieldsWithTypes): ComposableUrl {
  ...
  static fromBcs(data: Uint8Array): ComposableUrl {
    return ComposableUrl.fromFields(ComposableUrl.bcs.parse(data));
  }
  ...
  static fromJSONField(field: any): ComposableUrl {
    return ComposableUrl.reified().new({
      url: decodeFromJSONField(Url.reified(), field.url),
    });
  }

  static fromJSON(json: Record<string, any>): ComposableUrl {
  ...
  static fromSuiParsedData(content: SuiParsedData): ComposableUrl {
    if (content.dataType !== "moveObject") {
      throw new Error("not an object");
    }
    if (!isComposableUrl(content.type)) {
      throw new Error(
        `object at ${(content.fields as any).id} is not a ComposableUrl object`,
      );
    }
    return ComposableUrl.fromFieldsWithTypes(content);
  }

  static async fetch(client: SuiClient, id: string): Promise<ComposableUrl> {
    const res = await client.getObject({
      id,
      options: {
        showBcs: true,
    ...
    if (res.error) {
      throw new Error(
        `error fetching ComposableUrl object at id ${id}: ${res.error.code}`,
      );
    }
    if (
      res.data?.bcs?.dataType !== "moveObject" ||
      !isComposableUrl(res.data.bcs.type)
    ) {
