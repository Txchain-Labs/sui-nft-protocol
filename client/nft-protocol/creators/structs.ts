...
export class Creators implements StructClass {
  ...
  static reified(): CreatorsReified {
    return {
      ...
      typeArgs: [] as [],
      reifiedTypeArgs: [],
      fromFields: (fields: Record<string, any>) => Creators.fromFields(fields),
      fromFieldsWithTypes: (item: FieldsWithTypes) =>
        Creators.fromFieldsWithTypes(item),
      fromBcs: (data: Uint8Array) => Creators.fromBcs(data),
      bcs: Creators.bcs,
      fromJSONField: (field: any) => Creators.fromJSONField(field),
      fromJSON: (json: Record<string, any>) => Creators.fromJSON(json),
      fromSuiParsedData: (content: SuiParsedData) =>
        Creators.fromSuiParsedData(content),
      fetch: async (client: SuiClient, id: string) =>
        Creators.fetch(client, id),
      new: (fields: CreatorsFields) => {
        return new Creators([], fields);
      },
  ...
  static fromBcs(data: Uint8Array): Creators {
    return Creators.fromFields(Creators.bcs.parse(data));
  }
  ...
  static fromJSONField(field: any): Creators {
    return Creators.reified().new({
      creators: decodeFromJSONField(VecSet.reified("address"), field.creators),
    });
  }

  static fromJSON(json: Record<string, any>): Creators {
  ...
  static fromSuiParsedData(content: SuiParsedData): Creators {
    if (content.dataType !== "moveObject") {
      throw new Error("not an object");
    }
    if (!isCreators(content.type)) {
      throw new Error(
        `object at ${(content.fields as any).id} is not a Creators object`,
      );
    }
    return Creators.fromFieldsWithTypes(content);
  }

  static async fetch(client: SuiClient, id: string): Promise<Creators> {
    ...
    if (res.error) {
      throw new Error(
        `error fetching Creators object at id ${id}: ${res.error.code}`,
      );
    }
    if (
      res.data?.bcs?.dataType !== "moveObject" ||
      !isCreators(res.data.bcs.type)
    ) {
