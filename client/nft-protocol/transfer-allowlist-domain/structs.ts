...
export function isWitness(type: string): boolean {
  ...
  return (
    type ===
    "0xbc3df36be17f27ac98e3c839b2589db8475fa07b20657b08e8891e3aaf5ee5f9::transfer_allowlist_domain::Witness"
  );
}
...
export function isTransferAllowlistDomain(type: string): boolean {
  ...
  return (
    type ===
    "0xbc3df36be17f27ac98e3c839b2589db8475fa07b20657b08e8891e3aaf5ee5f9::transfer_allowlist_domain::TransferAllowlistDomain"
  );
}
...
export class TransferAllowlistDomain implements StructClass {
  ...
  static reified(): TransferAllowlistDomainReified {
    return {
      ...
      fromFields: (fields: Record<string, any>) =>
        TransferAllowlistDomain.fromFields(fields),
      fromFieldsWithTypes: (item: FieldsWithTypes) =>
        TransferAllowlistDomain.fromFieldsWithTypes(item),
      fromBcs: (data: Uint8Array) => TransferAllowlistDomain.fromBcs(data),
      bcs: TransferAllowlistDomain.bcs,
      fromJSONField: (field: any) =>
        TransferAllowlistDomain.fromJSONField(field),
      fromJSON: (json: Record<string, any>) =>
        TransferAllowlistDomain.fromJSON(json),
      fromSuiParsedData: (content: SuiParsedData) =>
        TransferAllowlistDomain.fromSuiParsedData(content),
      fetch: async (client: SuiClient, id: string) =>
        TransferAllowlistDomain.fetch(client, id),
      new: (fields: TransferAllowlistDomainFields) => {
        return new TransferAllowlistDomain([], fields);
      },
      kind: "StructClassReified",
    };
  }
  ...
  static fromFields(fields: Record<string, any>): TransferAllowlistDomain {
    return TransferAllowlistDomain.reified().new({
      allowlists: decodeFromFields(
  ...
  static fromFieldsWithTypes(item: FieldsWithTypes): TransferAllowlistDomain {
    if (!isTransferAllowlistDomain(item.type)) {
      throw new Error("not a TransferAllowlistDomain type");
    }

    return TransferAllowlistDomain.reified().new({
      allowlists: decodeFromFieldsWithTypes(
        VecSet.reified(ID.reified()),
        item.fields.allowlists,
      ),
    });
  }

  static fromBcs(data: Uint8Array): TransferAllowlistDomain {
  ...
  static fromJSONField(field: any): TransferAllowlistDomain {
    return TransferAllowlistDomain.reified().new({
      allowlists: decodeFromJSONField(
        VecSet.reified(ID.reified()),
        field.allowlists,
      ),
    });
  ...
  static fromJSON(json: Record<string, any>): TransferAllowlistDomain {
  ...
  static fromSuiParsedData(content: SuiParsedData): TransferAllowlistDomain {
    if (content.dataType !== "moveObject") {
      throw new Error("not an object");
    }
    if (!isTransferAllowlistDomain(content.type)) {
      throw new Error(
        `object at ${(content.fields as any).id} is not a TransferAllowlistDomain object`,
      );
    }
    return TransferAllowlistDomain.fromFieldsWithTypes(content);
  }
  ...
  static async fetch(
    ...
    if (res.error) {
      throw new Error(
        `error fetching TransferAllowlistDomain object at id ${id}: ${res.error.code}`,
      );
    }
    if (
      res.data?.bcs?.dataType !== "moveObject" ||
      !isTransferAllowlistDomain(res.data.bcs.type)
    ) {
      throw new Error(
        `object at id ${id} is not a TransferAllowlistDomain object`,
      );
    }
