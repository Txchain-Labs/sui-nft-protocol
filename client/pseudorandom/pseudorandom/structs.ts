...
export function isCounter(type: string): boolean {
  ...
  return (
    type ===
    "0x9e5962d5183664be8a7762fbe94eee6e3457c0cc701750c94c17f7f8ac5a32fb::pseudorandom::Counter"
  );
}
...
export class Counter implements StructClass {
  ...
  static reified(): CounterReified {
    return {
      ...
      fromFieldsWithTypes: (item: FieldsWithTypes) =>
        Counter.fromFieldsWithTypes(item),
      ...
      fromSuiParsedData: (content: SuiParsedData) =>
        Counter.fromSuiParsedData(content),
  ...
  static fromSuiParsedData(content: SuiParsedData): Counter {
    ...
    if (!isCounter(content.type)) {
      throw new Error(
        `object at ${(content.fields as any).id} is not a Counter object`,
      );
    }
