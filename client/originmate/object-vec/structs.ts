...
export function isObjectVec(type: string): boolean {
  ...
  return type.startsWith(
    "0xed6c6fe0732be937f4379bc0b471f0f6bfbe0e8741968009e0f01e6de3d59f32::object_vec::ObjectVec<",
  );
}
...
export class ObjectVec<V extends PhantomTypeArgument> implements StructClass {
  ...
  static reified<V extends PhantomReified<PhantomTypeArgument>>(
    ...
    return {
      ...
      typeArgs: [extractType(V)] as [
        PhantomToTypeStr<ToPhantomTypeArgument<V>>,
      ],
      reifiedTypeArgs: [V],
      fromFields: (fields: Record<string, any>) =>
        ObjectVec.fromFields(V, fields),
      fromFieldsWithTypes: (item: FieldsWithTypes) =>
        ObjectVec.fromFieldsWithTypes(V, item),
      fromBcs: (data: Uint8Array) => ObjectVec.fromBcs(V, data),
      bcs: ObjectVec.bcs,
      fromJSONField: (field: any) => ObjectVec.fromJSONField(V, field),
      fromJSON: (json: Record<string, any>) => ObjectVec.fromJSON(V, json),
      fromSuiParsedData: (content: SuiParsedData) =>
        ObjectVec.fromSuiParsedData(V, content),
      fetch: async (client: SuiClient, id: string) =>
        ObjectVec.fetch(client, V, id),
      new: (fields: ObjectVecFields<ToPhantomTypeArgument<V>>) => {
        return new ObjectVec([extractType(V)], fields);
      },
  ...
  static phantom<V extends PhantomReified<PhantomTypeArgument>>(
    ...
    return phantom(ObjectVec.reified(V));
  }
  ...
  static fromFields<V extends PhantomReified<PhantomTypeArgument>>(
    ...
    return ObjectVec.reified(typeArg).new({
  ...
  static fromBcs<V extends PhantomReified<PhantomTypeArgument>>(
    ...
    return ObjectVec.fromFields(typeArg, ObjectVec.bcs.parse(data));
  }
  ...
  static fromJSON<V extends PhantomReified<PhantomTypeArgument>>(
    ...
    assertReifiedTypeArgsMatch(
      composeSuiType(ObjectVec.$typeName, extractType(typeArg)),
      json.$typeArgs,
      [typeArg],
    );
  ...
  static fromSuiParsedData<V extends PhantomReified<PhantomTypeArgument>>(
    ...
    if (!isObjectVec(content.type)) {
      throw new Error(
        `object at ${(content.fields as any).id} is not a ObjectVec object`,
      );
    }
    return ObjectVec.fromFieldsWithTypes(typeArg, content);
  }
  ...
  static async fetch<V extends PhantomReified<PhantomTypeArgument>>(
    ...
    if (res.error) {
      throw new Error(
        `error fetching ObjectVec object at id ${id}: ${res.error.code}`,
      );
    }
    if (
      res.data?.bcs?.dataType !== "moveObject" ||
      !isObjectVec(res.data.bcs.type)
    ) {
