...
export function isEscrow(type: string): boolean {
  ...
  return type.startsWith(
    "0xed6c6fe0732be937f4379bc0b471f0f6bfbe0e8741968009e0f01e6de3d59f32::escrow_shared::Escrow<",
  );
}
...
export interface EscrowFields<T extends TypeArgument> {
  ...
  arbitrator: ToField<Option<"address">>;
  obj: ToField<Option<T>>;
}
...
export class Escrow<T extends TypeArgument> implements StructClass {
  ...
  readonly arbitrator: ToField<Option<"address">>;
  readonly obj: ToField<Option<T>>;
  ...
  private constructor(typeArgs: [ToTypeStr<T>], fields: EscrowFields<T>) {
    ...
    this.arbitrator = fields.arbitrator;
    this.obj = fields.obj;
  }
  ...
  static reified<T extends Reified<TypeArgument, any>>(
    ...
    return {
      ...
      reifiedTypeArgs: [T],
      fromFields: (fields: Record<string, any>) => Escrow.fromFields(T, fields),
      fromFieldsWithTypes: (item: FieldsWithTypes) =>
        Escrow.fromFieldsWithTypes(T, item),
      fromBcs: (data: Uint8Array) => Escrow.fromBcs(T, data),
      bcs: Escrow.bcs(toBcs(T)),
      fromJSONField: (field: any) => Escrow.fromJSONField(T, field),
      fromJSON: (json: Record<string, any>) => Escrow.fromJSON(T, json),
      fromSuiParsedData: (content: SuiParsedData) =>
        Escrow.fromSuiParsedData(T, content),
      fetch: async (client: SuiClient, id: string) =>
        Escrow.fetch(client, T, id),
      new: (fields: EscrowFields<ToTypeArgument<T>>) => {
        return new Escrow([extractType(T)], fields);
      },
  ...
  static phantom<T extends Reified<TypeArgument, any>>(
    ...
    return phantom(Escrow.reified(T));
  }
  ...
  static fromFields<T extends Reified<TypeArgument, any>>(
    ...
    return Escrow.reified(typeArg).new({
      id: decodeFromFields(UID.reified(), fields.id),
      sender: decodeFromFields("address", fields.sender),
      recipient: decodeFromFields("address", fields.recipient),
      arbitrator: decodeFromFields(
        Option.reified("address"),
        fields.arbitrator,
      ),
      obj: decodeFromFields(Option.reified(typeArg), fields.obj),
    });
  }
  ...
  static fromFieldsWithTypes<T extends Reified<TypeArgument, any>>(
    ...
    return Escrow.reified(typeArg).new({
      ...
      arbitrator: decodeFromFieldsWithTypes(
        Option.reified("address"),
        item.fields.arbitrator,
      ),
      obj: decodeFromFieldsWithTypes(Option.reified(typeArg), item.fields.obj),
    });
  }
  ...
  toJSONField() {
    return {
      ...
      arbitrator: fieldToJSON<Option<"address">>(
        `0x1::option::Option<address>`,
        this.arbitrator,
      ),
      obj: fieldToJSON<Option<T>>(
  ...
  static fromJSONField<T extends Reified<TypeArgument, any>>(
    ...
    return Escrow.reified(typeArg).new({
      ...
      arbitrator: decodeFromJSONField(
        Option.reified("address"),
        field.arbitrator,
      ),
      obj: decodeFromJSONField(Option.reified(typeArg), field.obj),
    });
  }
  ...
  static fromJSON<T extends Reified<TypeArgument, any>>(
    ...
    assertReifiedTypeArgsMatch(
      composeSuiType(Escrow.$typeName, extractType(typeArg)),
      json.$typeArgs,
      [typeArg],
    );
  ...
  static fromSuiParsedData<T extends Reified<TypeArgument, any>>(
    ...
    if (!isEscrow(content.type)) {
      throw new Error(
        `object at ${(content.fields as any).id} is not a Escrow object`,
      );
    }
    return Escrow.fromFieldsWithTypes(typeArg, content);
  }
  ...
  static async fetch<T extends Reified<TypeArgument, any>>(
    ...
    if (res.error) {
      throw new Error(
        `error fetching Escrow object at id ${id}: ${res.error.code}`,
      );
    }
    if (
      res.data?.bcs?.dataType !== "moveObject" ||
      !isEscrow(res.data.bcs.type)
    ) {
