...
export function isBox(type: string): boolean {
  ...
  return type.startsWith(
    "0xed6c6fe0732be937f4379bc0b471f0f6bfbe0e8741968009e0f01e6de3d59f32::box::Box<",
  );
}
...
export class Box<T extends TypeArgument> implements StructClass {
  ...
  static reified<T extends Reified<TypeArgument, any>>(
    ...
    return {
      ...
      reifiedTypeArgs: [T],
      fromFields: (fields: Record<string, any>) => Box.fromFields(T, fields),
      fromFieldsWithTypes: (item: FieldsWithTypes) =>
        Box.fromFieldsWithTypes(T, item),
      fromBcs: (data: Uint8Array) => Box.fromBcs(T, data),
      bcs: Box.bcs(toBcs(T)),
      fromJSONField: (field: any) => Box.fromJSONField(T, field),
      fromJSON: (json: Record<string, any>) => Box.fromJSON(T, json),
      fromSuiParsedData: (content: SuiParsedData) =>
        Box.fromSuiParsedData(T, content),
      fetch: async (client: SuiClient, id: string) => Box.fetch(client, T, id),
      new: (fields: BoxFields<ToTypeArgument<T>>) => {
        return new Box([extractType(T)], fields);
      },
  ...
  static fromFields<T extends Reified<TypeArgument, any>>(
    ...
    return Box.reified(typeArg).new({
  ...
  static fromJSON<T extends Reified<TypeArgument, any>>(
    ...
    assertReifiedTypeArgsMatch(
      composeSuiType(Box.$typeName, extractType(typeArg)),
      json.$typeArgs,
      [typeArg],
    );
  ...
  static fromSuiParsedData<T extends Reified<TypeArgument, any>>(
    ...
    if (!isBox(content.type)) {
      throw new Error(
        `object at ${(content.fields as any).id} is not a Box object`,
      );
    }
    return Box.fromFieldsWithTypes(typeArg, content);
  }
  ...
  static async fetch<T extends Reified<TypeArgument, any>>(
    ...
    if (res.error) {
      throw new Error(
        `error fetching Box object at id ${id}: ${res.error.code}`,
      );
    }
