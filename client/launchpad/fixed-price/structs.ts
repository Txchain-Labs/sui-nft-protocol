...
export function isMarketKey(type: string): boolean {
  ...
  return (
    type ===
    "0xc74531639fadfb02d30f05f37de4cf1e1149ed8d23658edd089004830068180b::fixed_price::MarketKey"
  );
}
...
export function isFixedPriceMarket(type: string): boolean {
  ...
  return type.startsWith(
    "0xc74531639fadfb02d30f05f37de4cf1e1149ed8d23658edd089004830068180b::fixed_price::FixedPriceMarket<",
  );
}
  ...
  implements StructClass
{
  ...
  private constructor(
    ...
    fields: FixedPriceMarketFields<FT>,
  ) {
    this.$fullTypeName = composeSuiType(
  ...
  static reified<FT extends PhantomReified<PhantomTypeArgument>>(
    ...
    return {
      ...
      fromFields: (fields: Record<string, any>) =>
        FixedPriceMarket.fromFields(FT, fields),
      fromFieldsWithTypes: (item: FieldsWithTypes) =>
        FixedPriceMarket.fromFieldsWithTypes(FT, item),
      fromBcs: (data: Uint8Array) => FixedPriceMarket.fromBcs(FT, data),
      bcs: FixedPriceMarket.bcs,
      fromJSONField: (field: any) => FixedPriceMarket.fromJSONField(FT, field),
      fromJSON: (json: Record<string, any>) =>
        FixedPriceMarket.fromJSON(FT, json),
      fromSuiParsedData: (content: SuiParsedData) =>
        FixedPriceMarket.fromSuiParsedData(FT, content),
      fetch: async (client: SuiClient, id: string) =>
        FixedPriceMarket.fetch(client, FT, id),
      new: (fields: FixedPriceMarketFields<ToPhantomTypeArgument<FT>>) => {
        return new FixedPriceMarket([extractType(FT)], fields);
      },
      kind: "StructClassReified",
    };
  }
  ...
  static phantom<FT extends PhantomReified<PhantomTypeArgument>>(
    ...
    return phantom(FixedPriceMarket.reified(FT));
  }
  ...
  static fromFields<FT extends PhantomReified<PhantomTypeArgument>>(
    ...
    return FixedPriceMarket.reified(typeArg).new({
  ...
  static fromJSON<FT extends PhantomReified<PhantomTypeArgument>>(
    ...
    assertReifiedTypeArgsMatch(
      composeSuiType(FixedPriceMarket.$typeName, extractType(typeArg)),
      json.$typeArgs,
      [typeArg],
    );
  ...
  static fromSuiParsedData<FT extends PhantomReified<PhantomTypeArgument>>(
    ...
    if (!isFixedPriceMarket(content.type)) {
      throw new Error(
        `object at ${(content.fields as any).id} is not a FixedPriceMarket object`,
      );
    }
    return FixedPriceMarket.fromFieldsWithTypes(typeArg, content);
  }
  ...
  static async fetch<FT extends PhantomReified<PhantomTypeArgument>>(
    ...
    if (res.error) {
      throw new Error(
        `error fetching FixedPriceMarket object at id ${id}: ${res.error.code}`,
      );
    }
    if (
      res.data?.bcs?.dataType !== "moveObject" ||
      !isFixedPriceMarket(res.data.bcs.type)
    ) {
