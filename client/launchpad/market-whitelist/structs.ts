...
export function isCertificate(type: string): boolean {
  ...
  return (
    type ===
    "0xc74531639fadfb02d30f05f37de4cf1e1149ed8d23658edd089004830068180b::market_whitelist::Certificate"
  );
}
...
export class Certificate implements StructClass {
  ...
  static reified(): CertificateReified {
    return {
      ...
      fromFields: (fields: Record<string, any>) =>
        Certificate.fromFields(fields),
      fromFieldsWithTypes: (item: FieldsWithTypes) =>
        Certificate.fromFieldsWithTypes(item),
      fromBcs: (data: Uint8Array) => Certificate.fromBcs(data),
      bcs: Certificate.bcs,
      fromJSONField: (field: any) => Certificate.fromJSONField(field),
      fromJSON: (json: Record<string, any>) => Certificate.fromJSON(json),
      fromSuiParsedData: (content: SuiParsedData) =>
        Certificate.fromSuiParsedData(content),
      fetch: async (client: SuiClient, id: string) =>
        Certificate.fetch(client, id),
      new: (fields: CertificateFields) => {
  ...
  static fromFields(fields: Record<string, any>): Certificate {
  ...
  static fromFieldsWithTypes(item: FieldsWithTypes): Certificate {
  ...
  static fromBcs(data: Uint8Array): Certificate {
    return Certificate.fromFields(Certificate.bcs.parse(data));
  }
  ...
  static fromJSONField(field: any): Certificate {
    return Certificate.reified().new({
  ...
  static fromJSON(json: Record<string, any>): Certificate {
  ...
  static fromSuiParsedData(content: SuiParsedData): Certificate {
    if (content.dataType !== "moveObject") {
      throw new Error("not an object");
    }
    if (!isCertificate(content.type)) {
      throw new Error(
        `object at ${(content.fields as any).id} is not a Certificate object`,
      );
    }
    return Certificate.fromFieldsWithTypes(content);
  }

  static async fetch(client: SuiClient, id: string): Promise<Certificate> {
    ...
    if (res.error) {
      throw new Error(
        `error fetching Certificate object at id ${id}: ${res.error.code}`,
      );
    }
    if (
      res.data?.bcs?.dataType !== "moveObject" ||
      !isCertificate(res.data.bcs.type)
    ) {
...
export function isWhitelist(type: string): boolean {
  ...
  return (
    type ===
    "0xc74531639fadfb02d30f05f37de4cf1e1149ed8d23658edd089004830068180b::market_whitelist::Whitelist"
  );
}
...
export class Whitelist implements StructClass {
  ...
  static reified(): WhitelistReified {
    return {
      ...
      fromFieldsWithTypes: (item: FieldsWithTypes) =>
        Whitelist.fromFieldsWithTypes(item),
      ...
      fromSuiParsedData: (content: SuiParsedData) =>
        Whitelist.fromSuiParsedData(content),
      fetch: async (client: SuiClient, id: string) =>
        Whitelist.fetch(client, id),
  ...
  static fromSuiParsedData(content: SuiParsedData): Whitelist {
    ...
    if (!isWhitelist(content.type)) {
      throw new Error(
        `object at ${(content.fields as any).id} is not a Whitelist object`,
      );
    }
  ...
  static async fetch(client: SuiClient, id: string): Promise<Whitelist> {
    ...
    if (res.error) {
      throw new Error(
        `error fetching Whitelist object at id ${id}: ${res.error.code}`,
      );
    }
    if (
      res.data?.bcs?.dataType !== "moveObject" ||
      !isWhitelist(res.data.bcs.type)
    ) {
