...
export function new_(
  ...
  return txb.moveCall({
    ...
    arguments: [pure(txb, rateBps, `u64`)],
  });
}

export interface CalcFeeArgs {
  proceedsValue: bigint | TransactionArgument;
  rateBps: bigint | TransactionArgument;
}

export function calcFee(txb: TransactionBlock, args: CalcFeeArgs) {
  return txb.moveCall({
    target: `${PUBLISHED_AT}::flat_fee::calc_fee`,
    arguments: [
      pure(txb, args.proceedsValue, `u64`),
      pure(txb, args.rateBps, `u64`),
    ],
  });
}
