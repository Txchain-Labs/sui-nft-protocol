...
export function isBid(type: string): boolean {
  ...
  return type.startsWith(
    "0xc74531639fadfb02d30f05f37de4cf1e1149ed8d23658edd089004830068180b::english_auction::Bid<",
  );
}
...
export function isMarketKey(type: string): boolean {
  ...
  return (
    type ===
    "0xc74531639fadfb02d30f05f37de4cf1e1149ed8d23658edd089004830068180b::english_auction::MarketKey"
  );
}
...
export class MarketKey implements StructClass {
  ...
  static reified(): MarketKeyReified {
    return {
      ...
      typeArgs: [] as [],
      reifiedTypeArgs: [],
      fromFields: (fields: Record<string, any>) => MarketKey.fromFields(fields),
      fromFieldsWithTypes: (item: FieldsWithTypes) =>
        MarketKey.fromFieldsWithTypes(item),
      fromBcs: (data: Uint8Array) => MarketKey.fromBcs(data),
      bcs: MarketKey.bcs,
      fromJSONField: (field: any) => MarketKey.fromJSONField(field),
      fromJSON: (json: Record<string, any>) => MarketKey.fromJSON(json),
      fromSuiParsedData: (content: SuiParsedData) =>
        MarketKey.fromSuiParsedData(content),
      fetch: async (client: SuiClient, id: string) =>
        MarketKey.fetch(client, id),
      new: (fields: MarketKeyFields) => {
        return new MarketKey([], fields);
      },
      kind: "StructClassReified",
    };
  }
  ...
  static fromFields(fields: Record<string, any>): MarketKey {
    return MarketKey.reified().new({
      dummyField: decodeFromFields("bool", fields.dummy_field),
    });
  }

  static fromFieldsWithTypes(item: FieldsWithTypes): MarketKey {
  ...
  static fromBcs(data: Uint8Array): MarketKey {
    return MarketKey.fromFields(MarketKey.bcs.parse(data));
  }
  ...
  static fromJSONField(field: any): MarketKey {
    return MarketKey.reified().new({
      dummyField: decodeFromJSONField("bool", field.dummyField),
    });
  }

  static fromJSON(json: Record<string, any>): MarketKey {
  ...
  static fromSuiParsedData(content: SuiParsedData): MarketKey {
    if (content.dataType !== "moveObject") {
      throw new Error("not an object");
    }
    if (!isMarketKey(content.type)) {
      throw new Error(
        `object at ${(content.fields as any).id} is not a MarketKey object`,
      );
    }
    return MarketKey.fromFieldsWithTypes(content);
  }

  static async fetch(client: SuiClient, id: string): Promise<MarketKey> {
    const res = await client.getObject({
      id,
      options: {
        showBcs: true,
    ...
    if (res.error) {
      throw new Error(
        `error fetching MarketKey object at id ${id}: ${res.error.code}`,
      );
    }
    if (
      res.data?.bcs?.dataType !== "moveObject" ||
      !isMarketKey(res.data.bcs.type)
    ) {
...
export function isEnglishAuction(type: string): boolean {
  type = compressSuiType(type);
  return type.startsWith(
    "0xc74531639fadfb02d30f05f37de4cf1e1149ed8d23658edd089004830068180b::english_auction::EnglishAuction<",
  );
}

export interface EnglishAuctionFields<
  T extends TypeArgument,
  FT extends PhantomTypeArgument,
> {
  nft: ToField<T>;
  bid: ToField<Bid<FT>>;
  concluded: ToField<"bool">;
}

export type EnglishAuctionReified<
  T extends TypeArgument,
  FT extends PhantomTypeArgument,
> = Reified<EnglishAuction<T, FT>, EnglishAuctionFields<T, FT>>;
...
> implements StructClass
{
  ...
  readonly nft: ToField<T>;
  readonly bid: ToField<Bid<FT>>;
  readonly concluded: ToField<"bool">;

  private constructor(
    typeArgs: [ToTypeStr<T>, PhantomToTypeStr<FT>],
    fields: EnglishAuctionFields<T, FT>,
  ) {
    this.$fullTypeName = composeSuiType(
    ...
    this.nft = fields.nft;
    this.bid = fields.bid;
    this.concluded = fields.concluded;
  }
  ...
  >(
    ...
    return {
      ...
      fromFields: (fields: Record<string, any>) =>
        EnglishAuction.fromFields([T, FT], fields),
      fromFieldsWithTypes: (item: FieldsWithTypes) =>
        EnglishAuction.fromFieldsWithTypes([T, FT], item),
      fromBcs: (data: Uint8Array) => EnglishAuction.fromBcs([T, FT], data),
      bcs: EnglishAuction.bcs(toBcs(T)),
      fromJSONField: (field: any) =>
        EnglishAuction.fromJSONField([T, FT], field),
      fromJSON: (json: Record<string, any>) =>
        EnglishAuction.fromJSON([T, FT], json),
      fromSuiParsedData: (content: SuiParsedData) =>
        EnglishAuction.fromSuiParsedData([T, FT], content),
      fetch: async (client: SuiClient, id: string) =>
        EnglishAuction.fetch(client, [T, FT], id),
      new: (
        fields: EnglishAuctionFields<
          ToTypeArgument<T>,
          ToPhantomTypeArgument<FT>
        >,
      ) => {
        return new EnglishAuction([extractType(T), extractType(FT)], fields);
      },
      kind: "StructClassReified",
    };
  }
  ...
  >(
    ...
    ToTypeStr<EnglishAuction<ToTypeArgument<T>, ToPhantomTypeArgument<FT>>>
  > {
    return phantom(EnglishAuction.reified(T, FT));
  }
  ...
  static get bcs() {
    return <T extends BcsType<any>>(T: T) =>
      bcs.struct(`EnglishAuction<${T.name}>`, {
        nft: T,
        bid: Bid.bcs,
        concluded: bcs.bool(),
      });
  ...
  >(
    ...
    return EnglishAuction.reified(typeArgs[0], typeArgs[1]).new({
      nft: decodeFromFields(typeArgs[0], fields.nft),
      bid: decodeFromFields(Bid.reified(typeArgs[1]), fields.bid),
      concluded: decodeFromFields("bool", fields.concluded),
    });
  }
  ...
  >(
    ...
    return EnglishAuction.reified(typeArgs[0], typeArgs[1]).new({
      nft: decodeFromFieldsWithTypes(typeArgs[0], item.fields.nft),
      bid: decodeFromFieldsWithTypes(Bid.reified(typeArgs[1]), item.fields.bid),
      concluded: decodeFromFieldsWithTypes("bool", item.fields.concluded),
    });
  }
  ...
  toJSONField() {
    return {
      nft: fieldToJSON<T>(this.$typeArgs[0], this.nft),
      bid: this.bid.toJSONField(),
  ...
  >(
    ...
    return EnglishAuction.reified(typeArgs[0], typeArgs[1]).new({
      nft: decodeFromJSONField(typeArgs[0], field.nft),
      bid: decodeFromJSONField(Bid.reified(typeArgs[1]), field.bid),
      concluded: decodeFromJSONField("bool", field.concluded),
    });
  }
  ...
  >(
    ...
    assertReifiedTypeArgsMatch(
      composeSuiType(EnglishAuction.$typeName, ...typeArgs.map(extractType)),
      json.$typeArgs,
      typeArgs,
    );
  ...
  >(
    ...
    if (!isEnglishAuction(content.type)) {
      throw new Error(
        `object at ${(content.fields as any).id} is not a EnglishAuction object`,
      );
    }
    return EnglishAuction.fromFieldsWithTypes(typeArgs, content);
  }
  ...
  >(
    ...
    if (res.error) {
      throw new Error(
        `error fetching EnglishAuction object at id ${id}: ${res.error.code}`,
      );
    }
    if (
      res.data?.bcs?.dataType !== "moveObject" ||
      !isEnglishAuction(res.data.bcs.type)
    ) {
