...
export function new_(txb: TransactionBlock, inner: ObjectArg) {
  return txb.moveCall({
    target: `${PUBLISHED_AT}::frozen_publisher::new`,
    arguments: [obj(txb, inner)],
  });
}

export function publicFreezeObject(txb: TransactionBlock, self: ObjectArg) {
  return txb.moveCall({
...
export function mod(txb: TransactionBlock, self: ObjectArg) {
  return txb.moveCall({
...
export function newDisplay(
  ...
  return txb.moveCall({
    ...
    arguments: [
      generic(txb, `${typeArgs[0]}`, args.parentWit),
      obj(txb, args.self),
    ],
  });
}

export function pkg(txb: TransactionBlock, self: ObjectArg) {
  return txb.moveCall({
